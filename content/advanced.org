...
The default preset is [[https://theme-ui.com/presets/deep][deep]]. To use a different one, simply install the package.
#+BEGIN_SRC sh
npm install @theme-ui/preset-dark
# or with yarn
yarn add @theme-ui/preset-dark
#+END_SRC
And change the option (see details about [[Config and Options][Config and Options]]) defined in your =gatsby-config.js=, like so.
